...
#else
        for (j = 0; j < nCentroid; j++) {
            dist = sqrt(pow(ConDens->flSamples[i][0] - vetCentroids[j].x, 2) + pow(ConDens->flSamples[i][1] - vetCentroids[j].y, 2));
            if (dist == 0)
...
void updateCondensation(IplImage* image, CvConDensation* ConDens, CvPoint *vetCentroids, int nCentroid, int show_particles) {
    ...
    resample(image, ConDens, vetCentroids, nCentroid, show_particles, &stdev_x, &stdev_y);
...
/*
void resample_old(IplImage* image, CvConDensation* ConDens, double measurement_x, double measurement_y, int show_particles, double *stdev_x, double *stdev_y){
    int i;
    double var_x, var_y, dist;
        double *sample_x;
        double *sample_y;
...
void updateCondensation_old(IplImage* image, CvConDensation* ConDens, double measurement_x, double measurement_y, int show_particles, double *predicted_x, double *predicted_y){
    double stdev_x, stdev_y;
        ...
        resample_old(image, ConDens, measurement_x, measurement_y, show_particles, &stdev_x, &stdev_y);
        cvConDensUpdateByTime(ConDens);
 *predicted_x = ConDens->State[0];
 *predicted_y = ConDens->State[1];
}
