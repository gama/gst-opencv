...
int ImageSourceDir::countFiles()
{
    ...
    glob("*.jpeg", 0, NULL, &globbuf);
    glob("*.jpg",  GLOB_APPEND, NULL, &globbuf);
    glob("*.png",  GLOB_APPEND, NULL, &globbuf);
    glob("*.bmp",  GLOB_APPEND, NULL, &globbuf);

    m_numFiles = globbuf.gl_pathc;
    for (int i = 0; i < m_numFiles; ++i)
    {
        m_fileNames[i] = string(globbuf.gl_pathv[i]);
    }
    globfree(&globbuf);
#endif
	return m_numFiles;
