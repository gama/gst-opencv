...
CvRect  motion_detect( IplImage* img, IplImage* motionHist)
{
    ...
    double count, count_comp_rect_bigger;
...
CvSeq*  motion_detect_mult( IplImage* img, IplImage* motionHist){
    ...
    for( i = 0; i < seq->total && i < MAX_MOTION_AREA; i++ ) indexDel[i] = 1;
    CvRect rect_i, rect_j;
    for( i = 0; i < seq->total && i < MAX_MOTION_AREA; i++ ) {
        ...
        for( j = i+1; j < seq->total && j < MAX_MOTION_AREA; j++ ) {
            ...
            CvRect rect_sum = cvMaxRect(&rect_i, &rect_j);
